...
Upload the spectra
```{r}
new.spectrum <- read.xlsx(xlsxFile = "~/Supplementary Table 1. Sample description for C.elegans experiments.xlsx", sheet = 3, rowNames = T)
...
```{r}
Y <- new.spectrum
CD2Mutant_tmp <- CD2Mutant[data$Drug.concentration > 0 | data$Generation > 3]
...
z <- 'polh-1:UV'
tmp <- (Y[rev(grep(z,CD2Mutant[rownames(Y)])),])
dss <- data$Drug.concentration[match(rownames(tmp),data$Sample)]
tmp <- tmp[order(dss, decreasing = T),]
dss <- dss[order(dss, decreasing = T)]
pdf(paste0(gsub(z,pattern = ':',replacement = '.'),'.barplot.pdf'),6,5)
f <- barplot(t(cbind(rowSums(tmp[,1:16]),rowSums(tmp[,17:32]),rowSums(tmp[,33:48]),
        ...
        main = z, horiz = T, yaxt = 'n', xlim = c(0,40), border = NA)
axis(side = 2, at = f, labels = dss, las = 2)
dev.off()
```
